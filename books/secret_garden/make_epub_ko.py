"""
make_epub_ko.py
Compiles the Korean version of The Secret Garden into an EPUB file.
"""
...
OUTPUT_FILE   = os.path.join(BASE_DIR, "secret_garden_ko.epub")
...
def txt_to_html(text, title):
    ...
    for line in lines:
        ...
        if not line:
            continue
        # Skip the chapter title if it's already the first line (e.g. 제1장 제목)
        if line == title or (line.startswith("제") and "장" in line and (line.split("장", 1)[0] + "장").strip() == title):
            continue
...
def main():
    ...
    # 1. Introduction
    intro_txt_path = os.path.join(BASE_DIR, "overview_ko.txt")
    if os.path.exists(intro_txt_path):
        intro_text = read_txt(intro_txt_path)
        intro_html = txt_to_html(intro_text, "독자 여러분께")
        chapters.append({'id': 'intro', 'href': 'Text/intro.xhtml', 'title': '독자 여러분께', 'content': intro_html})
    ...
    for i in range(1, 28):
        ...
        title = first_line if first_line.startswith(f"제{i}장") else f"제 {i} 장"
    ...
    if os.path.exists(copy_txt_path):
        ...
        copy_html = txt_to_html(copy_text, "판권 및 에디션 소개")
        chapters.append({'id': 'copyright', 'href': 'Text/copyright.xhtml', 'title': '판권 및 에디션 소개', 'content': copy_html})
...
<body>
  <img src="../Images/cover.png" alt="Cover" />
    ...
    opf_parts = [
        ...
        "    <dc:title>비밀의 화원</dc:title>",
        "    <dc:language>ko</dc:language>",
        "    <dc:creator>프랜시스 호지슨 버넷</dc:creator>",
        "    <dc:publisher>TKPROF LLC</dc:publisher>",
        "    <dc:date>2026-07-17</dc:date>",
        "    <dc:description>인도에서 홀로 자란 심술궂고 병약한 소녀 메리 레녹스는 콜레라로 고아가 되어 영국 요크셔의 미셀스웨이트 저택에 보내집니다. 그곳에서 10년 동안 잠겨 있던 비밀의 정원을 발견하고, 동물들과 소통하는 소년 디콘, 몸이 아픈 사촌 콜린과 함께 정원을 함께 가꾸며 치유와 성장의 마법 같은 기적을 경험하게 됩니다.</dc:description>",
    ...
    ncx_parts = [
        ...
        "  <head>",
        "    <meta name=\"dtb:uid\" content=\"tkprof-secret-garden-ko-2026\"/>",
    ...
    nav_parts = [
        ...
        "    nav a { text-decoration: none; color: #1e3a1e; }",
        "  </style>",
        "</head>",
        "<body>",
        ...
        "    <h1>목차</h1>",
        "    <ol>",
        "      <li><a href=\"Text/cover.xhtml\">표지</a></li>"
    ]
