...
def clean_text(text):
    text = re.sub(r'</?(?:mary|colin|dickon|martha|craven|ben|others)>', '', text)
    text = re.sub(r'\s+', '', text)
    text = re.sub(r'[^\w]', '', text)
    return text.lower()

def align_tags_with_difflib(original, tagged_output):
    tag_pattern = re.compile(r'<(mary|colin|dickon|martha|craven|ben|others)>(.*?)</\1>', re.DOTALL)
    stripped_text = ""
    tagged_ranges = []
...
def tag_dialogue_with_gemini(chapter_text, chapter_name, lang):
    model_name = "gemini-flash-lite-latest"  # Use flash-lite model to avoid rate limit blocks
    
    if lang == "ko":
        char_desc = (
            "- <mary>...</mary> for Mary Lennox (메리 레녹스)\n"
            "- <colin>...</colin> for Colin Craven (콜린 크레이븐)\n"
            "- <dickon>...</dickon> for Dickon Sowerby (디콘 소어비)\n"
            "- <martha>...</martha> for Martha Sowerby / Susan Sowerby (마사 소어비 / 수잔 소어비)\n"
            "- <craven>...</craven> for Archibald Craven (아치볼드 크레이븐)\n"
            "- <ben>...</ben> for Ben Weatherstaff (벤 웨더스태프)\n"
            "- <others>...</others> for other characters (Mrs. Medlock/메들록 부인, Dr. Craven/크레이븐 의사, Nurse/간호사, etc.)"
        )
        system_instruction = (
            "You are an expert literary assistant specializing in tagging dialogue.\n"
            "Your task is to read the provided chapter of The Secret Garden in Korean, identify all spoken dialogue, and wrap the spoken text in XML-style speaker tags, WHILE KEEPING all non-spoken narration text exactly as is.\n"
            f"Available speaker tags:\n{char_desc}\n\n"
            "CRITICAL RULES:\n"
        ...
        char_desc = (
            "- <mary>...</mary> for Mary Lennox\n"
            "- <colin>...</colin> for Colin Craven\n"
            "- <dickon>...</dickon> for Dickon Sowerby\n"
            "- <martha>...</martha> for Martha Sowerby / Susan Sowerby\n"
            "- <craven>...</craven> for Archibald Craven\n"
            "- <ben>...</ben> for Ben Weatherstaff\n"
            "- <others>...</others> for other characters (Mrs. Medlock, Dr. Craven, Nurse, etc.)"
        )
        system_instruction = (
            "You are an expert literary assistant specializing in tagging dialogue.\n"
            "Your task is to read the provided chapter of The Secret Garden in English, identify all spoken dialogue, and wrap the spoken text in XML-style speaker tags, WHILE KEEPING all non-spoken narration text exactly as is.\n"
            f"Available speaker tags:\n{char_desc}\n\n"
            "CRITICAL RULES:\n"
            "1. ONLY wrap the actual spoken dialogue (usually inside English double quotation marks “...” or \"...\") in the tags. Keep all narration and non-spoken text completely intact and unchanged. Do NOT omit any narrative paragraphs or sentences.\n"
            "2. Keep the quotation marks inside or outside the speaker tags as they are, but make sure the exact spoken words are wrapped.\n"
            "3. DO NOT modify, translate, paraphrase, omit, or add a single character, word, punctuation, spacing, or paragraph from the original chapter text. The final output must be identical to the input chapter, word-for-word, character-for-character, with only the opening and closing speaker tags added around the spoken dialogue.\n"
            "4. Return the entire chapter with the speaker tags. Do not include any summary, introductory remarks, or markdown block formatting (do NOT wrap the output in ``` or ```xml)."
        )
    ...
    while attempt < 5:
        try:
            ...
            # Clean all tags from result
            cleaned_result = re.sub(r'</?(?:mary|colin|dickon|martha|craven|ben|others)>', '', result)
            ...
            if orig_normalized == cleaned_normalized:
                ...
                tag_words = ["<mary>", "<colin>", "<dickon>", "<martha>", "<craven>", "<ben>", "<others>"]
                if not any(tag in result for tag in tag_words):
                ...
                try:
                    ...
                    cleaned = re.sub(r'</?(?:mary|colin|dickon|martha|craven|ben|others)>', '', reconstructed)
            ...
            if "Quota exceeded" in err_str or "429" in err_str or "RESOURCE_EXHAUSTED" in err_str:
                quota_retries += 1
                if quota_retries > 15:
                    raise RuntimeError(f"Rate limits hit 15 times consecutively on {chapter_name}. Exiting.")
    ...
    raise RuntimeError(f"Failed to process {chapter_name} due to errors/mismatches after retries.")
...
def main():
    ...
    # We want to process both English and Korean chapters
    for lang in ["en", "ko"]:
        print(f"\nProcessing {lang.upper()} chapters...")
        chapter_files = sorted(glob.glob(os.path.join(input_dir, f"ch_*_{lang}.txt")))
        ...
        for file_path in chapter_files:
            ...
            # If already processed and clean, skip
            if os.path.exists(out_file_path):
                ...
                if clean_text(chapter_text) == clean_text(existing_tagged):
                    print(f"Skipping {file_name} (already tagged and valid)")
                    continue
                    
            print(f"Processing {file_name} -> {out_file_name}...")
            tagged_text = tag_dialogue_with_gemini(chapter_text, file_name, lang)
