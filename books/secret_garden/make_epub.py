"""
make_epub.py
Compiles The Secret Garden into an EPUB file directly using python's zipfile.
...
OUTPUT_FILE   = os.path.join(BASE_DIR, "secret_garden.epub")
...
def txt_to_html(text, title):
    ...
    for line in lines:
        ...
        if not line:
            continue
        # Skip the chapter title if it's already the first line (e.g. Chapter 1: Title)
        if line == title or (":" in line and line.split(":", 1)[0].strip() == title):
            continue
...
def main():
    ...
    for i in range(1, 28):
        ...
        # Extract title from first line of text if formatted as "Chapter X: Title"
        first_line = text.split("\n", 1)[0].strip()
        title = first_line if first_line.startswith(f"Chapter {i}") else f"Chapter {i}"
...
<body>
  <img src="../Images/cover.png" alt="Cover" />
    ...
    opf_parts = [
        ...
        "    <dc:title>The Secret Garden</dc:title>",
        "    <dc:language>en</dc:language>",
        "    <dc:creator>Frances Hodgson Burnett</dc:creator>",
        "    <dc:publisher>TKPROF LLC</dc:publisher>",
        "    <dc:date>2026-07-17</dc:date>",
        "    <dc:description>Mary Lennox, a sickly and sour young girl, is orphaned and sent to live in Yorkshire. There, she discovers a locked, hidden garden, and through caring for it alongside a local boy named Dickon and her bedridden cousin Colin, she finds health, friendship, and the magic of hope.</dc:description>",
    ...
    ncx_parts = [
        ...
        "  <head>",
        "    <meta name=\"dtb:uid\" content=\"tkprof-secret-garden-2026\"/>",
        "    <meta name=\"dtb:depth\" content=\"1\"/>",
        "    <meta name=\"dtb:totalPageCount\" content=\"0\"/>",
        "    <meta name=\"dtb:maxPageNumber\" content=\"0\"/>",
        "  </head>",
        "  <docTitle>",
        "    <text>The Secret Garden</text>",
        "  </docTitle>",
        "  <navMap>"
    ]
    play_order = 1
