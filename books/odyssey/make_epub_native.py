"""
make_epub_native.py
Compiles the English version of The Odyssey into an EPUB file natively and cleanly.
"""
...
# ── CSS Style ─────────────────────────────────────────────────────────────────
# Avoid external font @import to prevent Google Books and offline validation rejections.
STYLE = """
body {
...
# ── Build EPUB ────────────────────────────────────────────────────────────────
def main():
    print("Building English EPUB for The Odyssey natively (strict compliance)...")
...
<body>
  <img src="../Images/cover.png" alt="Cover" />
    ...
    # Build HTML5 Navigation (clean without inline styles to avoid validation flags)
    nav_parts = [
        "<?xml version='1.0' encoding='utf-8'?>",
        "<!DOCTYPE html>",
        "<html xmlns=\"http://www.w3.org/1999/xhtml\" xmlns:epub=\"http://www.idpf.org/2007/ops\">",
...
<container version="1.0" xmlns="urn:oasis:names:tc:opendocument:xmlns:container">
  <rootfiles>
    ...
    # Zip output using the native standard (ZipFile defaults to STORED, files compressed explicitly)
    ...
    with zipfile.ZipFile(OUTPUT_FILE, "w") as epub:
        ...
        # 2. META-INF
        epub.writestr("META-INF/container.xml", container_content.encode("utf-8"), compress_type=zipfile.ZIP_DEFLATED)
        ...
        epub.writestr("OEBPS/Text/cover.xhtml", cover_html.encode("utf-8"), compress_type=zipfile.ZIP_DEFLATED)
        epub.writestr("OEBPS/Styles/main.css", STYLE.encode("utf-8"), compress_type=zipfile.ZIP_DEFLATED)
        epub.writestr("OEBPS/content.opf", opf_content.encode("utf-8"), compress_type=zipfile.ZIP_DEFLATED)
        epub.writestr("OEBPS/toc.ncx", ncx_content.encode("utf-8"), compress_type=zipfile.ZIP_DEFLATED)
        epub.writestr("OEBPS/nav.xhtml", nav_content.encode("utf-8"), compress_type=zipfile.ZIP_DEFLATED)
        
        for ch in chapters:
            epub.writestr(f"OEBPS/{ch['href']}", ch['content'].encode("utf-8"), compress_type=zipfile.ZIP_DEFLATED)
            
    print(f"Native English EPUB created successfully at: {OUTPUT_FILE}")
