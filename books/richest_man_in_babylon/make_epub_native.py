"""
make_epub_native.py
Compiles The Richest Man in Babylon into an EPUB file directly using python's zipfile.
...
OUTPUT_FILE   = os.path.join(BASE_DIR, "the_richest_man_in_babylon.epub")
...
def txt_to_html(text, title):
    ...
    for line in lines:
        ...
        if line == title or line.startswith("Chapter"):
            if line.lower() == title.lower() or line.lower().startswith(title.lower()):
                continue
...
def main():
    ...
    # 2. Chapters 1-11
    for i in range(1, 12):
        # Find the file matching ch_XX_*_en.txt
        pattern = os.path.join(CHAPTERS_DIR, f"ch_{i:02d}_*_en.txt")
        matches = glob.glob(pattern)
        if not matches:
            # Try without _en.txt just in case
            pattern2 = os.path.join(CHAPTERS_DIR, f"ch_{i:02d}_*.txt")
            matches = glob.glob(pattern2)
            # filter out part1, part2 etc.
            matches = [m for m in matches if 'part' not in m]
            
        if not matches:
            print(f"Skipping chapter {i}, no file found.")
            continue
...
<body>
  <img src="../Images/cover.png" alt="Cover" />
    ...
    opf_parts = [
        ...
        "    <dc:title>The Richest Man in Babylon</dc:title>",
        "    <dc:language>en</dc:language>",
        "    <dc:creator>George S. Clason</dc:creator>",
        "    <dc:publisher>TKPROF LLC</dc:publisher>",
        "    <dc:date>2026-07-02</dc:date>",
        "    <dc:description>Beloved by millions, this timeless classic holds the key to all you desire and everything you wish to accomplish. Modernized for the casual reader and listener.</dc:description>",
    ...
    ncx_parts = [
        ...
        "  <head>",
        "    <meta name=\"dtb:uid\" content=\"tkprof-richest-man-in-babylon-2026\"/>",
        "    <meta name=\"dtb:depth\" content=\"1\"/>",
        "    <meta name=\"dtb:totalPageCount\" content=\"0\"/>",
        "    <meta name=\"dtb:maxPageNumber\" content=\"0\"/>",
        "  </head>",
        "  <docTitle><text>The Richest Man in Babylon</text></docTitle>",
        "  <navMap>"
    ]
