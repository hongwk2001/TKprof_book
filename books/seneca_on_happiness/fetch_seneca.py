...
def fetch_and_segment_all_chapters():
    ...
    for idx, roman in enumerate(ROMAN_NUMERALS, 1):
        ...
        url = f"https://en.wikisource.org/wiki/Of_a_Happy_Life/Book_{roman}"
        ...
        try:
            ...
            content_html = re.sub(r'<div class="ws-header.*?">.*?</div>', '', content_html, flags=re.DOTALL)
            ...
            # Strip license and other boilerplate text if present at the end
            body_text = "\n\n".join(clean_paragraphs)
            # Remove any trailing wikisource template / copyright boilerplates if any
            body_text = re.sub(r'This work is in the public domain.*', '', body_text, flags=re.DOTALL).strip()
            
            full_text = f"Chapter {roman}\n\n{body_text}\n"
