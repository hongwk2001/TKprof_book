...
for tag in soup.find_all(['span', 'p', 'tr']):
    ...
    if re.fullmatch(r'[IVXLC]+', text):
        ...
        tds = tag.find_all('td')
        if len(tds) >= 3:
            en_text = tds[2].get_text(' ', strip=True)
            if en_text:
                if not chapters[current_chapter] or chapters[current_chapter][-1] != en_text:
                    chapters[current_chapter].append(en_text)

for chap, lines in chapters.items():
    if lines:
        with open(os.path.join(out_dir, f'{chap}_en.txt'), 'w', encoding='utf-8') as f:
            f.write('\n'.join(lines))
